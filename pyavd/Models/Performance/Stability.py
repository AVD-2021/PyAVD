...
class Stability(Model):
    ...
    def get_etah(self):
        """get_etah
            Argument: Baseline config to get tailplane configuration
            Return: Eta H - horizontal stabilizer efficiency
        """

        # to be subbed by actual taiplane from baseline config
        tailplane = "t_tail"
        ...
        elif tailplane == "low_tail":
            return 0.9

        else: return None   # We want to break something


    def calc_fuseCM(fuse_length, fuse_max_width, c_bar, wing_ref_area, qc_root_pos):
        """calc_fuseCM [summary]
            Arguments:  fuselage length [meters], fuse_max_width [meters],
                        wing_mean_chord [meters], wing_ref_area [meters^2],
                        position of 1/4 chord length at wing root [% body length]

            Return: Pitching moment generated by fuselage about its CG
        """

        K_f = None # TODO: look for this later

        return K_f * fuse_length * (fuse_max_width**2) / (c_bar * wing_ref_area)


    def calc_deta_dalpha(wing_ar, wing_taper_ratio, rel_tail_h, rel_tail_l, wing_span, wing_sweep, TBD):
        ...
        K_a = (1.0/wing_ar) - 1.0/(1.0+wing_ar**1.7)
